  ...
  return (
    <main className="min-h-screen bg-[#081030] text-white">
      <div className="max-w-6xl mx-auto px-6 pt-16 pb-10 flex flex-col md:flex-row items-center justify-between gap-10">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-300 mt-6 max-w-2xl text-[15px] leading-relaxed border-l-2 border-purple-500 pl-4">
            BCA Final Year Student (2024-2027) @ BRABU | MERN Stack Developer (React, Next.js) | Looking for new opportunities | Muzaffarpur, Bihar | Passionate about building web applications.
          </p>
        </div>
      ...
      <footer className="w-full text-center py-8 mt-10 border-t border-white/10">
        <p className="text-white text-sm">Made in India with <span className="text-red-500">❤️</span> by Himanshu Kumar</p>
...
function FacebookTool(){ const [name,setName]=useState("Himanshu Kumar"); const [bio,setBio]=useState(""); return <><h3 className="font-bold mb-3">Facebook Bio</h3><input value={name} onChange={e=>setName(e.target.value)} className="w-full p-3 rounded-xl bg-black/40" /><button onClick={()=>setBio(`🔥 ${name} | BCA Final Year | MERN Developer | Muzaffarpur, Bihar`)} className="w-full mt-4 bg-white text-blue-900 p-3 rounded-xl font-bold">Generate</button>{bio && <textarea value={bio} readOnly className="w-full mt-3 p-3 rounded-xl bg-black/40 h-24 text-sm"></textarea>}</>}